...
Addresses are `/explorer/{character set}/{word}`, for example `/explorer/traditional/學`. Supported sets: simplified (the default for a first visit to `/explorer/`; subsequent bare launches remember the last language), traditional, cantonese, and japanese. The word can be any search, sentences included; it is one encoded path segment (`parseExplorerPath` and `explorerPath` in `immersive-card.mjs`).

The address bar follows the open card, so a reload or shared link returns to it, and Back and Forward step through the cards you opened. Restoring a word uses the same graph framing and forced word connections as searching it. Opening a card pushes a history entry; Back, Forward, and the first load only replace, so they never erase Forward history. Closing a card opened from the bare graph steps back rather than stacking a second bare-graph entry. Panning and zooming never touch history. Deep paths rely on the `/explorer/**` rewrite, so run the explorer locally with the Firebase hosting emulator (`firebase emulators:start --only hosting --project demo-hanzigraph`, port 5002); `python3 -m http.server` and `scripts/server_with_rewrites.py` don't route `/explorer/…`. The emulator answers conditional requests for rewritten URLs from a stale ETag after an edit, so on localhost the explorer's worker refetches instead of revalidating.

## Installable app

The explorer installs as its own app, separate from classic HanziGraph: `public/explorer/manifest.json` (start URL `/explorer/`, which remembers the last language, scope `/explorer/`) and its own worker, `public/explorer/sw.js`. Because the worker script sits in `/explorer/`, browsers confine it to that path: it never controls a classic page, and it only deletes caches named `explorer-*`. A `firebase.json` rewrite serves this page for every `/explorer/…` path.

Pages load network-first (every request the worker sends revalidates with the server, so the browser's HTTP cache can't answer from a stale guess), falling back to the saved copy offline or after 4 seconds on a stalled connection, so a deploy shows on the next launch. Code and data answer from the cache and refresh in the background; a new `?v=` build replaces the old entry. On localhost everything is network-first. Bump `cacheName` in `sw.js` only when the caching scheme itself changes. Offline use covers whatever has been opened online.

Classic's `/asset-service-worker.js` deletes only its own old `hanzigraph-*` caches, preserving Explorer's offline data. In a browser that already has classic's worker, the explorer's very first load goes through it, so classic's cache may keep a copy of those files; the explorer's worker handles subsequent controlled loads.
...
Choose **Japanese** in the language / character set menu, or open `/explorer/japanese/` (optionally `/explorer/japanese/学校`). This uses the same explorer, renderer budget, search connection behavior, and detail cards. Classic links point to `https://japanesegraph.com/japanese/<word>`; speech uses `ja-JP`. The canvas contains kanji only, while dictionary searches and cards may include kana-only words.
